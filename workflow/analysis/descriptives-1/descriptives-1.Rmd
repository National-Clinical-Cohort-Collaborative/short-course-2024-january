...
1. The current report covers `r nrow(ds_person)` patients, with `r dplyr::n_distinct(ds_person$data_partner_id)` unique values for `data_partner_id`.
...
```{r marginals-patient-hidden, echo=echo_chunks, message=message_chunks}
```
